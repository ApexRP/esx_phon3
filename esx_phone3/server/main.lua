...
function GenerateUniquePhoneNumber()
	...
	while not foundNumber do
		...
		phoneNumber = "06" .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9) .. math.random(9)
...
AddEventHandler('esx_phone:tryOpenPhone', function()
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('iphone').count >= 1 then
			TriggerClientEvent('esx_phone:openPhone', src)
		else
			TriggerClientEvent("pNotify:SendNotification", source, { text = "Du har ingen mobil!" })
		end
	end
end)
...
AddEventHandler('esx_phone:tryOpenPhoneCalling', function(target, channel, number)
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('iphone').count >= 1 then
			TriggerClientEvent('esx_phone:openPhoneCalling', src, target, channel, number)
		else
  ...
  for num,v in pairs(PhoneNumbers) do
	if tonumber(num) == num then -- If phonenumber is a player phone number
	  for src,_ in pairs(v.sources) do
		TriggerClientEvent('esx_phone:setPhoneNumberSource', src, num, tonumber(src))
	  end
	end
  end
...
AddEventHandler('esx_phone:removePlayerContact', function(phoneNumber)
	...
	if xPlayer ~= nil then
		...
		TriggerClientEvent("pNotify:SendNotification", source, { text = "Du raderar denna kontakt" })
	end
end)
	...
	function(result)
	  ...
	  if foundNumber then
		...
		if phoneNumber == xPlayer.get('phoneNumber') then
		  TriggerClientEvent("pNotify:SendNotification", source, { text = "Du kan inte lägga till dig själv" })
		else
		  ...
		  if hasAlreadyAdded then
			TriggerClientEvent("pNotify:SendNotification", source, { text = "Kontakten finns redan i din katalog" })
		  else
			...
			MySQL.Async.execute(
			  ...
			  function(rowsChanged)
				TriggerClientEvent("pNotify:SendNotification", source, { text = "Kontakt tillagd" })
		...
		TriggerClientEvent("pNotify:SendNotification", targetPlayer["source"], { text = "Du mottog en swish på " .. amount .. "kr" })
	  end
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
	...
	TriggerClientEvent("pNotify:SendNotification", source, { text = "Du swishade " .. amount .. "SEK" })

  else
	TriggerClientEvent("pNotify:SendNotification", source, { text = "Ogiltigt belopp" })
  end
