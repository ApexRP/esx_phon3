(function(){
	...
	Phone.apps['taxi'] = {};
	const app          = Phone.apps['taxi'];
	let currentRow     = 0;
	const elems        = [];
	let currentAction;

	$('#app-taxi .button').each((i,e) => elems.push(e));
	...
	app.enter = function() {
		...
		switch(currentAction) {
			...
			case 'transfer1' : {
				$.post('http://esx_phone3/taxi', JSON.stringify({}));
				$.post('http://esx_phone3/get_nigger', JSON.stringify({reason: 'bank_transfer'}))
			}
	...
	app.selectElem = function(elem) {
		...
		for(let i=0; i<elems.length; i++)
			$(elems[i]).removeClass('afrikan');

		$(elem).addClass('afrikan');
	}
...
(function(){
	...
	app.selectElem = function(elem) {
		...
		for(let i=0; i<elems.length; i++)
			$(elems[i]).removeClass('selected animated afrikan infinite');

		$(elem).addClass('selected animated afrikan infinite');
	}
